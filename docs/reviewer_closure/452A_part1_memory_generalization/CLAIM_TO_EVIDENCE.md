...
| C5 | In the **non-saturated** regime (budget < top-k), CCR = budget/top-k and is **invariant** to memory size and composition. | non-saturated campaign (100 A01 b1/b2 runs) | b1 CCR 0.333 [.333,.333]; b2 CCR 0.667 [.667,.667] — all profiles |
...
| C7 | Denser / more-episodic / higher-similarity memory does **not** reduce success — the reviewer's hypothesis is refuted. | C4 + C5 + C6 across EPISODIC_HEAVY, DENSE, BENIGN_HIGHSIM | zero variance across profiles at each budget |
| C8 | Controls behave correctly (no false positives). | 50 A00 b0 runs | CCR 0.0 all profiles/seeds |

## Headline claim (scoped)
> CCR = budget/top-k, invariant to memory size (3–200) and composition, at top-k = 3,
