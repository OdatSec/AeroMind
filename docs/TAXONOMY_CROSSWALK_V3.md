...
## Raw results layout (future runs, opt-in `--results-layout v3`)
```
results_v3_raw/
  <ATTACK>/<TASK>/<MEMORY>/<EVALUATION>/<MODEL>/<DEFENSE>/seed-XXXX/<run-id>/
```
Short `seed-XXXX` and `run-<cfg8>-<uuid8>` dirs; **all** config hashes, UUIDs,
commit IDs, canonical + legacy IDs live in `manifest.json`, not the path.
`results_v3_raw/` is git-ignored (like the v2 raw bundles).

## Campaign layer (human-readable)
```
results_v3_campaigns/
  INDEX.md            (auto)          PAPER_FINDINGS.md  (human-only, never auto)
  <ATTACK>_<TASK>_<MEMORY>_<EVALUATION>/
    README.md  campaign_summary.json  paired_results.csv  bundle_index.yaml
    INSIGHTS_DRAFT.md (auto DRAFT)     CLAIMS.md
```
`INSIGHTS_DRAFT.md` is generated by `uavsys/campaigns.build_campaign`; promotion
into `PAPER_FINDINGS.md` is a **human-only** step.
